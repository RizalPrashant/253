...
void init_buffer()
{
    ...
    int i;
    signal(SIGALRM, dump_buffer);  // creating signal
    alarm(alarm_interval);		// creating alarm
...
static void dump_buffer()
{
  FILE *myFile = fopen(log_name,"w");
  int i;
  for(i = 0; i < MAX_LOG_ENTRY; i++){
   fprintf(myFile,"log %d: %s\n",i, buff.log[i]);   //print those things but to a file named myFile. 
  }
  fclose(myFile);
